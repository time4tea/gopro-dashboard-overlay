...
def journey_map(at, entry, privacy_zone, renderer, timeseries, roundedcorners, transparencylevel, size=256):
    return JourneyMap(
        at=at,
...
def moving_map(at, entry, size, zoom, renderer, roundedcorners, transparencylevel):
    return MovingMap(
        at=at,
        location=lambda: entry().point,
